<div>
    ...
    <div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#1A1A2E] border-t border-purple-500/10 shadow-lg">
        <nav class="flex justify-around items-center p-3">
            @foreach ($menuItems as $item)
                <a href="{{ $item['path'] }}" class="flex flex-col items-center justify-center space-y-1 {{ ($item['path'] === '/' && request()->is('/')) || request()->is(trim($item['path'], '/')) ? 'text-purple-400' : 'text-gray-400' }}">
