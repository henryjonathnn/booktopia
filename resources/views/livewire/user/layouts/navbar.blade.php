<nav class="fixed top-0 right-0 left-0 md:left-20 glass-effect z-40 border-b border-purple-500/10">
    <div class="flex items-center justify-between px-4 md:px-8 py-4"> <!-- Logo --> <a href="/"
            class="text-2xl font-bold bg-gradient-to-r from-purple-400 to-indigo-400 bg-clip-text text-transparent">
            Booktopia </a> <!-- Search (Desktop) -->
        <div class="hidden md:flex flex-1 max-w-2xl mx-12">
            <div class="relative w-full"> <input wire:model.live.debounce.500ms="search" type="text"
                    placeholder="Cari buku, penulis, atau genre.."
                    class="w-full bg-[#1A1A2E] rounded-xl pl-12 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-purple-500/5 border border-purple-500/10" />
                <span class="absolute left-4 top-3.5 h-5 w-5 text-gray-500"> <svg wire:loading.remove
                        wire:target="search" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                    </svg> <svg wire:loading wire:target="search" class="animate-spin"
                            ...
                            @if (count($searchResults) > 0)
                                @foreach ($searchResults as $book)
                                    <div wire:click="viewBook({{ $book->id }})"
                                        ...
                                        {{-- Sampul Gambar --}} <div
                                            class="w-12 h-16 flex-shrink-0 rounded overflow-hidden bg-gray-800 mr-3">
                                            @if ($book->cover_img)
                                            ...
                                            @endif
                                        </div> {{-- Info buku --}} <div class="flex-1 min-w-0">
                                            <p class="text-white font-medium truncate">{{ $book->judul }}</p>
                                            <p class="text-sm text-gray-400 truncate">{{ $book->penulis }}</p> <span
                                                class="inline-block mt-1 text-xs px-2 py-0.5 bg-purple-500/20 text-purple-400 rounded-full">{{ $book->kategori }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <div class="p-4 text-center text-gray-400"> <svg class="w-6 h-6 mx-auto mb-2"
                                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                                    </svg> Tidak ada hasil untuk "{{ $search }}" </div> @endif
                        </div>
                    </div> @endif
            </div>
        </div> <!-- Icon Menu -->
        <div class="flex items-center space-x-6"> <!-- Notifikasi Dropdown -->
            <div class="relative"> <button wire:click="toggleNotifikasi"
                    class="relative p-2 hover:bg-[#2a2435] rounded-lg text-gray-400"> <svg class="w-6 h-6"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                @if ($unreadCount > 0)
                                    <button wire:click="markAllAsRead"
                                        class="text-sm text-purple-400 hover:text-purple-300"> Tandai semua telah dibaca
                                    </button>
                                    @endif
                            </div>
                        ...
                        <div class="max-h-96 overflow-y-auto">
                            @forelse($notifikasi as $notif)
                                <div wire:key="notif-{{ $notif->id }}"
                                    ...
                                    <div class="flex items-start gap-3"> <!-- Icon berdasarkan tipe -->
                                        <div @class([
                                            ...
                                            'bg-yellow-500/10 text-yellow-400' =>
                                                $notif->tipe === 'PEMINJAMAN_TERLAMBAT',
                                            'bg-gray-500/10 text-gray-400' =>
                                                $notif->tipe === 'PEMINJAMAN_DIKEMBALIKAN',
                                        ])> @switch($notif->tipe)
                                                ...
                                                @case('PEMINJAMAN_DIKIRIM')
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M5 13l4 4L19 7M13 3v18m0-18l6 6m-6-6L7 9" />
                                                    </svg>
                                                @break
                                        ...
                                        </div> <button wire:click="showDetail({{ $notif->id }})"
                                            class="flex-shrink-0 text-gray-400 hover:text-gray-300"> <svg
                                ...
                                </div> @empty <div class="p-8 text-center">
                                        <div
                                            class="w-16 h-16 mx-auto mb-4 rounded-full bg-purple-500/10 flex items-center justify-center">
                                            <svg class="w-8 h-8 text-purple-400" fill="none" viewBox="0 0 24 24"
                                                stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                                            </svg> </div>
                ...
                @if ($user)
                    <div class="relative"> <button wire:click="toggleProfileDropdown"
                            ...
                            <div class="w-8 h-8 rounded-full overflow-hidden border-2 border-purple-500">
                                ...
                                @else
                                    <div class="w-full h-full bg-purple-500 flex items-center justify-center"> <x-icon
                                            name="user" class="h-5 w-5 text-white" /> </div>
                                    @endif
                            </div> <x-icon name="chevron-down" class="h-4 w-4 text-gray-400" />
                        </button>
                        @if ($isProfileDropdownOpen)
                            <div
                                ...
                                <div class="flex items-center space-x-3 px-4 py-3 border-b border-purple-500/10">
                                    <div class="w-10 h-10 rounded-full overflow-hidden border-2 border-purple-500">
                                        ...
                                        @else
                                            <div class="w-full h-full bg-purple-500 flex items-center justify-center">
                                                <x-icon name="user" class="h-6 w-6 text-white" /> </div>
                                        @endif
                                    </div>
                                    <div>
                                    ...
                                    class="block w-full text-left px-4 py-2 hover:bg-purple-500/10 text-sm"> Profil Saya
                                </a> <a href="/peminjaman"
                                    class="block w-full text-left px-4 py-2 hover:bg-purple-500/10 text-sm"> Peminjaman
                                    Saya </a> <button wire:click="logout"
                                    class="w-full text-left px-4 py-2 hover:bg-purple-500/10 text-red-500 text-sm"> Logout
                                </button>
                            </div>
                        @endif
                    </div>
            ...
            <div class="flex min-h-screen items-end justify-center p-4 text-center sm:items-center sm:p-0">
                ...
                <div
                    ...
                    <div>
                        ...
                        ])> <!-- Icon sesuai tipe --> @switch($selectedNotifikasi->tipe)
                                @case('PEMINJAMAN_BARU')
                        ...
                        <div class="mt-3 text-center sm:mt-5">
                            <h3 class="text-lg font-medium leading-6 text-gray-200"> Detail Notifikasi </h3>
                            <div class="mt-2">
                                <p class="text-sm text-gray-400"> {{ $selectedNotifikasi->message }} </p>
                                            ...
                                            @if ($selectedNotifikasi->peminjaman->buku->cover_img)
                                                ...
                                                @endif <div class="flex-1 min-w-0">
                                                    <p class="text-sm font-medium text-gray-200">
                                                        {{ $selectedNotifikasi->peminjaman->buku->judul }} </p>
                                                    <p class="text-sm text-gray-400">
                                                        {{ $selectedNotifikasi->peminjaman->buku->penulis }} </p>
                                                </div>
                                        </div>
                                        @if ($selectedNotifikasi->peminjaman->bukti_pengiriman && $selectedNotifikasi->tipe === 'PEMINJAMAN_DIKIRIM')
                                            <div class="mt-4">
                                                <p class="text-sm text-gray-400 mb-2">Bukti Pengiriman:</p> <img
                                                    src="{{ Storage::url($selectedNotifikasi->peminjaman->bukti_pengiriman) }}"
                                                    alt="Bukti Pengiriman" class="max-h-64 mx-auto rounded-lg">
                                            </div>
                                        @endif
                                    </div>
                                @endif
                    ...
                    <div class="mt-5 sm:mt-6"> <button wire:click="closeDetail"
